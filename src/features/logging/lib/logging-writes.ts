...
type DailyLogRpcArgs = Database["public"]["Functions"]["save_daily_health_record_log"]["Args"];
type DailyRecordLogValues = Partial<Omit<DailyLogRpcArgs, "p_cat_id" | "p_record_date">>;
...
function mapFoodValues(values: FormValues): DailyRecordLogValues {
...
function mapActivityValues(values: FormValues): DailyRecordLogValues {
  const activityScore =
...
function mapAbnormalValues(values: FormValues): DailyRecordLogValues {
  const eventType = typeof values.eventType === "string" ? values.eventType : "other";
  const repeated = values.repeatedToday === true;

  return {
    p_vomit_times: eventType === "vomiting" ? (repeated ? 2 : 1) : null,
...
function mapMedicationValues(values: FormValues): DailyRecordLogValues {
  const status =
    values.status === "given"
      ? "taken"
...
function mapWeightValues(values: FormValues): DailyRecordLogValues {
  return {
...
export async function saveLogEntry(category: LogItemId, values: FormValues) {
  ...
  const dailyRecordPayload: DailyLogRpcArgs = {
    p_cat_id: catId,
  ...
  // ### keep accumulation server-side so repeated same-day logs cannot overwrite prior health signals
