...
- Alert evaluation now uses a server-side debounce model:
  ...
  - worker Edge Functions require a service-role bearer token before any service-role reads or writes
  - the logging UI, dashboard, and profile surfaces can show that today's logs are being reviewed
- Same-day quick-log submissions now merge into the existing `daily_health_record` for that cat and date:
  - `vomit_times` accumulates across repeated vomiting logs
  - gram-based `food_amount_grams` accumulates across same-day food logs
  - new notes append to existing same-day notes instead of replacing earlier context
  - unrelated daily fields from other quick-log categories are preserved
