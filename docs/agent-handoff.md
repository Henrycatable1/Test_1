...
- Alert evaluation now uses a server-side debounce model:
  ...
  - scheduled worker requests use the Supabase Vault secret named `service_role_key`
  - Edge Functions reject public anon invocations before service-role reads, writes, or emails
...
- Same-day quick-log aggregation is implemented for the confirmed MVP fields:
  - `vomit_times` accumulates across same-day vomiting submissions
  - non-vomiting abnormal logs no longer reset same-day vomiting counts
  - gram-based food logs accumulate into the daily food total
  - notes append so separate same-day quick-log context is not silently erased
  - see `docs/repeated-event-logging-gap.md` before extending same-day quick-log write behavior
