...
- Alert evaluation now uses a server-side debounce model:
  ...
  - `pg_cron` schedules `process-pending-alert-checks` every 30 seconds using Vault secrets `project_url` and `service_role_key`
  - Edge Function alert workers require the service role bearer token so public anon clients cannot trigger cross-cat jobs
  - queue claims use a short lease and new log activity clears prior claims so a timed-out worker cannot block a cat's future alert evaluation indefinitely
...
- Same-day quick-log aggregation status:
  - confirmed accumulating fields now merge against the existing daily row for normal sequential saves
  - `vomit_times` accumulates, `food_amount_grams` accumulates, and same-day quick-log notes append instead of being erased by later blank notes
  - `urine_times` remains a single daily value for MVP 1.0
  - see `docs/repeated-event-logging-gap.md` before changing or expanding same-day quick-log write behavior
