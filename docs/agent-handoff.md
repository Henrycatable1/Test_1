...
- Alert evaluation now uses a server-side debounce model:
  ...
  - scheduled alert-worker calls must use the Supabase Vault `service_role_key` secret in the bearer token because the functions perform service-role writes
