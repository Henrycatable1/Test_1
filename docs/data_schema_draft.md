...
### `check-alerts`
Responsibilities:
- require a private service-role invocation, not the public anon key
...
Responsibilities:
- require a private service-role invocation from the scheduled worker
...
### `send-alert-digests`
Responsibilities:
- require a private service-role invocation from the scheduled digest worker
- run daily
