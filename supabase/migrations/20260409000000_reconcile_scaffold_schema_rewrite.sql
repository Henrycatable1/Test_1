-- ### reconcile rewritten 20260408 bootstrap for DBs that already applied the scaffold version
--
-- Why this exists:
-- Commit 7269f9f replaced the contents of already-versioned `20260408_initial_schema.sql`.
-- Databases that recorded that version against the old prototype schema never receive the
-- CATable bootstrap, and `20260408235800_add_cat_health_context.sql` is a no-op.
-- This forward migration runs before `20260421100000_*` so `app_private` and core tables exist.
-- On databases that already have the current schema, the transforms no-op and the remaining
-- DDL is idempotent (`if not exists` / `create or replace` / `drop if exists`).
...
-- ### detect and reshape databases that applied the pre-rewrite scaffold bootstrap
-- Concrete failure mode without this migration:
-- 1) DB applied scaffold `20260408` (profiles/cats/log_entries, no app_private)
-- 2) repo rewrite made `20260408` a full CATable bootstrap, but version is already recorded
-- 3) `20260408235800` is a no-op, then `20260421100000` fails on missing app_private

-- ### drop scaffold-era RLS policies only when those relations still exist
do $$
begin
  ...
  if to_regclass('public.cats') is not null then
    execute 'drop policy if exists "cats_access_by_owner" on public.cats';
  end if;

  if to_regclass('public.log_entries') is not null then
    execute 'drop policy if exists "log_entries_access_by_owner" on public.log_entries';
  end if;

  if to_regclass('public.medication_plans') is not null then
    execute 'drop policy if exists "medication_plans_access_by_owner" on public.medication_plans';
  end if;

  if to_regclass('public.follow_up_tasks') is not null then
    execute 'drop policy if exists "follow_up_tasks_access_by_owner" on public.follow_up_tasks';
  end if;
end $$;

-- ### reshape legacy profiles toward the CATable shape when the table already exists
do $$
begin
  ...
  alter table public.profiles
    ...
    add column if not exists last_vet_visit_self date,
    add column if not exists years_owning_cat integer,
    add column if not exists household_cat_count integer,
    add column if not exists updated_at timestamptz;

  update public.profiles
  set language_code = coalesce(language_code, 'zh-TW'),
  ...
  begin
    alter table public.profiles
      ...
      check (language_code in ('en', 'zh-TW'));
  exception
  ...
  begin
    alter table public.profiles
      add constraint profiles_years_owning_cat_check
      check (years_owning_cat is null or years_owning_cat >= 0);
  exception
    when duplicate_object then null;
  end;
  ...
  begin
    alter table public.profiles
      add constraint profiles_household_cat_count_check
      check (household_cat_count is null or household_cat_count >= 0);
  exception
    when duplicate_object then null;
  end;

  -- ### attach auth.users FK when missing and data allows it
  if not exists (
    select 1
    from pg_constraint
    ...
    exception
      when others then
        raise notice 'profiles_id_fkey not added during reconcile: %', sqlerrm;
    end;
  end if;
end $$;

-- ### rebuild legacy cats (profile_id/age_years/sex/weight_kg) into CATable cats
do $$
begin
  if to_regclass('public.cats') is null then
    return;
  end if;
  ...
  if exists (
    ...
    create table public.cats_reconcile (
    ...
    insert into public.cats_reconcile (
      id,
      owner_user_id,
      name,
      age_months,
    ...
    select
      ...
      c.profile_id,
      c.name,
      greatest(c.age_years, 0) * 12,
      case lower(c.sex)
        when 'male' then 'male'::public.cat_gender
        when 'female' then 'female'::public.cat_gender
        -- scaffold allowed 'unknown'; CATable has no equivalent, so default conservatively
        else 'female'::public.cat_gender
      end,
      nullif(c.breed, ''),
      c.weight_kg,
      nullif(c.personality, ''),
      c.created_at,
      timezone('utc', now())
    from public.cats c;

    drop table public.cats cascade;
    alter table public.cats_reconcile rename to cats;
  end if;
end $$;

-- ### retire prototype-only tables that the rewritten bootstrap no longer uses
