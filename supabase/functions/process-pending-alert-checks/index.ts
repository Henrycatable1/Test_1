...
import { authorizeAlertWorkerRequest, getAlertWorkerSecret } from "../_shared/worker-auth.ts";
...
const alertWorkerSecret = getAlertWorkerSecret();
...
async function runAlertCheck(catId: string) {
  const response = await fetch(checkAlertsUrl, {
    ...
    headers: {
      ...
      ...(alertWorkerSecret ? { "x-alert-worker-secret": alertWorkerSecret } : {}),
    },
    body: JSON.stringify({
      catId,
    }),
...
serve(async (request) => {
  const unauthorized = authorizeAlertWorkerRequest(request);
