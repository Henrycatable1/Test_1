...
import { hasServiceRoleAuthorization, unauthorizedWorkerResponse } from "../_shared/worker-auth.ts";
...
async function loadDueRows(referenceTime: string) {
  const staleClaimCutoff = new Date(Date.parse(referenceTime) - staleClaimMs).toISOString();
...
serve(async (request) => {
  if (!hasServiceRoleAuthorization(request, serviceRoleKey)) {
